...
At this point at the end of video 23, we have code that kinda looks like:
...
with Presentational Components, and Container Components using lifecycle methods
...
- Context has an "unstable API, so best not to use."
...
## Generating Containers for ES6 Class Containers with connect() from React Redux (video 27)
...
## Generating Containers for ES6 Functional Component Containers with connect() from React Redux (video 27)
...
We just need the dispatch, not the whole store.

We will read in dispatch function through props,

and inject dispatch into Container function using `connect()`:
```
...
Here is all the code for our new, "generated" AddTodo:
```
// Container.
let AddTodo = ({ dispatch }) => {
  let input
...
// dispatch prop will be generated by component with connect() call.
AddTodo = connect(
  // 1. mapStateToProps: returns empty object because
  // there are no props for AddTodo that depend on current state.
  state => {
    return {};
  },
  // 2. mapDispatchToProps: but AddTodo component doesn't need any callback props,
  // it just accepts the dispatch function itself, so return as prop w/ same name.
  dispatch => {
    return { dispatch };
  }
